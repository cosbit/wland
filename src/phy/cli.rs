...
use crate::phy::get_wireless_phy;
    ...
    pub fn run(self) -> Result<()> {
        match self.command {
            PhyCommand::Show { phy_name } => {
                ...
                let state = runtime.block_on(get_wireless_phy(&phy_name))?;
                print_state(&state);
                Ok(())
            }
        }
...
fn print_state(state: &crate::phy::WirelessPhyState) {
    ...
    println!(
        "    path:        {}",
        state.identity.path.as_deref().unwrap_or("none")
    );
    println!(
        "    driver:      {}",
        state.identity.driver.as_deref().unwrap_or("none")
    );
    println!(
        "    mac:         {}",
        state.identity.mac.as_deref().unwrap_or("none")
    );
    println!(
        "    bus_path:    {}",
        state.identity.bus_path.as_deref().unwrap_or("none")
    );
    ...
    println!(
        "    channels:    {}",
        state.capabilities.supported_channels.len()
    );
